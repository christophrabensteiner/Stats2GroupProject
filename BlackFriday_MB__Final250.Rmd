---
  title: "Black Friday - eine statistische Analyse der Verkaufszahlen"
  output:
...
Der Datensatz wurde bereitgestellt durch einen  [Hackathon der Firma AnalyticsVidhya](https://datahack.analyticsvidhya.com/contest/black-friday/) und heruntergeladen über [Kaggle.com](https://www.kaggle.com/mehdidag/black-friday).
...
Struktur der Daten: Die Funktion str() gibt viele nützliche Informationen zurück, einschließlich den Dimensionen des Dataframes, die enthaltenen Feature Namen, deren Datentype und die ersten 10 Beobachtungen pro Feature.  In diesem Beispiel bedeutet "int", dass die Variable "User_ID" eine ganze Zahl ist, und "Factor" bedeutet, dass die Variable "Product_ID" mit 3623 Kategorien oder Ebenen kategorisch ist. 
```{r}
str(bf)
```

Zusammenfassung der Daten: Die Funktion summary() wird im Wesentlichen auf jede Spalte eines Data Frames angewendetund und die Ergebnisse werden für alle Spalten werden gemeinsam angezeigt. Für die Integer Variablen gibt die Funktion die Auflistung in Minimum, 1. Quantile, Median, Mean, 3. Quantile und Maximum zurück. Würden NA Werte auftreten, wie für die Variable "Product_Categorie_2"", dann werden auch diese als Summe aufgelistet. Für eine kategorische Variable wie "Product_ID" werden die Ebenen und die Anzahl der Daten in jeder Ebene zurückgegeben.
```{r}
summary(bf)
```
...
`Purchase`: Summe des Einkaufs: Hierzu fehlt leider gänzlich die Information um die Währung. Wir nehmen an, dass das Dollar Summen sind, obwohl uns diese ersichtlich hoch vorkommen. Der Maximal Wert liegt hier bei 23.961$. Wir vermuten dennoch, dass es sich hierbei um Ausreißer in im amerikanischen Black Firday Markt handelt und vereinzelt auch vorkommen kann. 
...
Bei der Überprüfung der NAs fällt uns auf, dass wir nur in der Product Category 2 & 3 NA-Werte enthalten haben. In der Beschreibung zum Datensatz befinden sich keine Informatione dazu, wie die NA-Werte zu interpretieren sind. Nach Betrachtung der Daten haben wir folgende Annhame getroffen: Sind NA in einem Sample enthalten, gehen wir davon aus, dass von dieser Produktkategorie kein Produkt gekauft wurde. Das konnten wir überprüfen, da es keine "0" Werte in Category 2 & 3 gibt. Somit nehmen wir an, dass die NA-Werte für "nicht gekauft" steht, und ersetzen aus diesem Grund die NA-Werte mit 0. 

NA-Werte sind immer schlecht in einem Datensatz, da diese Werte die Samples unter Umständen unbrauchbar machen können. Durch das Ersetzen mit 0 können wir alle Samples behalten und auch nicht-gekaufte Produkte repräsentiern.

Zusätzlich werden die Produktkategorien von Numeric in Integer umgewandelt:
```{r}
bf[is.na(bf)] <- 0
...
grid.arrange(tableGrob(unique), bottom = textGrob("Grafik 1: Tabelle, Anzahl eindeutiger Variablen"))
...
Geschlechterverteilung  = ggplot(data =Geschlecht_Subset) +
                          ...
                          labs(y='Anzahl', x='Geschlecht', title = 'Geschlechterverteilung der Kunden',  caption = 'Grafik 2:  Barplot, Geschlechterverteilung der Kunden') + 
...
Wir haben insgesamt 1.666 weibliche und 4.225 männliche Kunden. Somit können wir sagen, dass wir einen unbalanzierten Datensatz haben, da die männlichen Konsumenten 3-mal öfters vorkommen als die weiblichen. Eine mögliche Erklärung für dieses Ungleichgewicht wäre, dass das Geschäft für männliche Käufer ausgelegt bzw. attraktiver gestaltet wurde und weibliche Konsumenten weniger anspricht.
...
grid.arrange(tableGrob(gender_split), bottom = textGrob("Grafik 3: Tabelle, Gender Split"))
```


Nachfolgenden untersuchen wir den monitären Wert der Einkäufe auf die Geschlachte abgebildet: 

```{r, echo=F}
bf %>% ggplot(aes(x=Gender, y=Purchase, fill=Gender))+
  geom_boxplot() + 
  scale_fill_brewer(palette = 'Paired') +
  labs(x='Geschlecht', y='Wert der Einkäufe',  caption = 'Grafik 4:  Boxplot, Geschlechterverteilung der Kunden')

```

In unserem Boxplot erkennen wir, dass es Ausreißer be nach oben gibt, die sich im Bereich der 20k-25k ansiedeln. Allgemein gilt es sich zu überlegen, ob Ausreißer aus dem Datensatz entfernt werden sollen oder nicht. 
Wir entscheiden uns die Ausreißer nicht zu entfernen, da wir annehmen, dass an einem Black Friday Event Ausreißer sehr wohl vorkommen können. Die Verteilung des Einkäufswerts nach Geschlecht ist visuell betrachtet ähnlich. Das heißt beide Geschlechter kaufen für mindestens 5k ein, wo bei es bei den Männern mehr Beobachtungen gibt, die über 11k aber bis 12k ausgeben. Geschuldet kann das der 75%:25% Geschlechterverteilung sein.

Da im Boxplot oben (Boxplot der Eikäufe nach Geschlecht) der Durchschnitt der Einkäufe je Geschlecht sehr ähnlich ist, führen wir einen t-Test durch, um zu überprüfen, ob die Verteilung der Einkäufe nach Geschlecht signifikant unterschiedlich ist. Grundsätzlich gehen wir davon aus, dass die Daten aufgrund des Grenzwertsatzes Normalverteilt sind, da es einen genügend großen Stichprobenumfang (500.000 Personen) gibt. 
In unserem Fall verwenden wir den Zweistichproben-t-Test um zu Überprüfen, ob der Mittelwert beider Stichproben gleich ist. Die Nullhypothese sagt hier, dass die Mittelwerte beider Gruppen gleich sind (Konfidenzintervall 95%):

```{r}
t.test(bf$Purchase~bf$Gender, conf.level = 0.95)
```

Da der p-Wert kleiner als 0.05 ist, kann die Nullhypothese verworfen werden und es gilt, dass der Mittelwert beider Gruppen signifikant unterschiedlich ist. Wir können sagen, dass die männliche Kunden durchschnittlich mehr ausgegen als weibliche Kunden.
...
ggplot(data=gender_split) + 
  ...
  labs(x='Geschlecht', y='Durchschnitt',title = "Durchschnittliche Ausgaben je Geschlecht", caption = 'Grafik 5:  Barplot, Durchschnittliche Ausgaben je Geschlecht') + 
  scale_fill_brewer(palette = 'Paired')
```
...
grid.arrange(tableGrob(head(bestverkaufte_Produkte,10)), bottom = textGrob("Grafik 6: Tabelle, Best verkauftestes Produkt"))
```
...
```{r,echo=F}
ggplot(bestes_Produkt) + 
  geom_bar(mapping = aes(x=Gender, y=..count..,fill=Gender)) + 
  labs(x='Geschlecht', y='Anzahl',title= "Verteilung des Geschlechts am meistverauften Produkt", caption = 'Grafik 7:  Barplot, Verteilung des Geschlechts am meistverauften Produkt') +
  scale_fill_brewer(palette = 'Paired')
```
...
grid.arrange(ggplot_best  + theme(legend.position="none") , ggplot_all  + theme(legend.position="none"), ncol=2, bottom = textGrob("Grafik 8: Barplots, Geschlechterverteilung- bestes Produkt || \n Geschlechterverteilung aller Produkte"))
```
...
Im Histogramm (`Grafik 9:  Histogramm, Histogramm der Berufsgruppen`) sehen wir, dass die erste Berufgruppe am Häufigsten vorkommt, gefolgt von der vierten und siebten Berufsgruppen:
...
ggplot(bf) +
  ...
  labs(title="Histogramm der Berufsgruppen",x="Berufsgruppen", y = "Häufigkeit", caption = 'Grafik 9:  Histogramm, Histogramm der Berufsgruppen') + 
...
ggplot(Occupation_PurchaseAmount, mapping = aes(x=Occupation, y=SummeEinkauf, fill=Occupation)) + 
  geom_col() + 
  scale_fill_manual(values = getPalette(colourCount)) +
  labs(x='Berufsgruppen', y='Summe der Einkäufe', caption = 'Grafik 10:  Histogramm, Summe der Einkäufe je Berufsgruppe')
```

Die Summe der Einkäufe ist bei der Berufsgruppe 0, 4 und 7 am höchsten. Daraus lässt sich ableiten, dass die Berufsgruppen die am häufigsten vertreten sind, auch am meisten einkaufen. Da keine Informationen zu der Bedeutung der Zahlen der Berufsgruppen vorhanden sind, ist der Informationsgehalt der obigen Grafik (`Grafik 10: Histogramm, Summe der Einkäufe je Berufsgruppe`) eher gering.
...
grid.arrange(tableGrob(cat_split), bottom = textGrob("Grafik 11: Tabelle, Produktkategorien je Geschlecht"))
```
...
ages = bf %>%
  ...
  distinct() %>%
  count(Age)
grid.arrange(tableGrob(ages), bottom = textGrob("Grafik 12: Tabelle, Alter der Kunden"))
```
...
```{r, echo=F}
ggplot(data=ages) + 
  geom_bar(mapping=aes(x=Age,y=n, fill=Age), stat='identity') +
  labs(x='Altersgruppe', y='Anzahl', title = "Anzahl der Kunden je Altersgruppe", caption = 'Grafik 13:  Barplot, Anzahl der Kunden je Altersgruppe')+ 
  scale_fill_brewer(palette = 'Paired')
```


In der Grafik (`Grafik 13:  Barplot, Anzahl der Kunden je Altersgruppe`) wird ersichtlich, dass die meisten Käufer zwischen 26-35 und 36-45 Jahre alt sind.


## Wohnort

In diesem Teil beschäftigen wir uns mit der Analyse des Wohnortes unserer Kunden und gehen auf folgende Fragen ein:
...
grid.arrange(ggplot_Wohnort + theme(legend.position="none"), ggplot_SummeEkWohnort + theme(legend.position="none"), ncol=2, bottom = textGrob("Grafik 14: Barplots, Wohnort der Kunden ||\n Summe der Einkäufe nach Wohnort (monetär)"))
```
 

Obwohl die meisten Kunden aus `Wohnort C` stammen, haben die Kunden aus `Wohnort B` in Summe deutlich mehr für ihre Einkäufe ausgegeben, was anhand der oberen Grafiken (`Grafik 14: Barplots, Wohnort der Kunden || Summe der Einkäufe nach Wohnort (monetär)`) ersichtlich ist. 
...
```{r,echo=F}
grid.arrange(ggplotAnzahlEinkaeufeJeStadt + theme(legend.position="none"), ggplot_SummeEkWohnort + theme(legend.position="none"), ncol=2, bottom = textGrob("Grafik 15: Barplots,Summe der Artikel nach Wohnort ||\n Summe der Einkäufe nach Wohnort (monetär)"))
```
...
grid.arrange(ggplot_SummeEkWohnort  + theme(legend.position="none"),ggplotBestesProdJeStadt + theme(legend.position="none"), ncol = 2, bottom = textGrob("Grafik 16: Barplots, Summe der Einkäufe nach Wohnort (monetär) ||\n Anzahl Verkauf Bestes Produkt je Stadt"))
```
...
ggplot_KundeWohnzeit = ggplot(data = Kunde_Wohnzeit, aes(x = Stay_In_Current_City_Years, y = ..count.., fill = Stay_In_Current_City_Years)) +
                              geom_bar(stat = 'count') +
                              scale_fill_brewer(palette = 'Paired') +
                              labs(title = 'Bisheriger Verbleib am Wohnort', y = 'Summe', x = 'Verbleib in der gegenwärtigen Stadt', fill = 'Anzahl Jahre am Wohnort', caption = 'Grafik 17:  Barplot, Bisheriger Verbleib am Wohnort')
ggplot_KundeWohnzeit
```

Anhand der oberen Grafik ('Grafik 17:  Barplot, Bisheriger Verbleib am Wohnort') ist abzulesen, dass die meisten Kunden an ihrem jeweiligen Standort gegenwärtig 1 Jahr leben, jedoch bezieht sich diese Inforamtion auf alle drei Wohnorte kummoliert. Eine Erklärung wäre, dass alle drei Wohnorte vor einem Jahr eine einen starken "Zuzug" erlebt hatten. Eine alternative Interpretation kann aber auch sein, dass in diesen Wohnorten die meisten Bürger nach einem Jahr erneut umziehen. Dies ist jedoch mit den gegebenen Daten nicht eindeutig bestimmbar.
...
ggplot_wohnzeit_stadt_abs = ggplot(data = wohnzeit_stadt, aes(x = City_Category, y = n, fill = Stay_In_Current_City_Years)) + 
    geom_bar(stat = "identity", color = 'white') + 
    scale_fill_brewer(palette = 'Paired') +
    labs(title = "Verbleib am Wohnort in Abhängigkeit zum \n Wohnort", 
            y = "Summe aller Bewohner", 
            x = "Wohnort", 
            fill = "Verbleib in Jahren")
#ggplot_wohnzeit_stadt_abs


ggplot_wohnzeit_stadt_rel = ggplot(data = wohnzeit_stadt, aes(x = City_Category, y = Percentage, fill = Stay_In_Current_City_Years)) + 
    geom_bar(stat = "identity", color = 'white') + 
    scale_fill_brewer(palette = 'Paired') +
    labs(title = "Verbleib am Wohnort in Abhängigkeit zum \n Wohnort - normiert", 
            y = "Summe aller Bewohner", 
            x = "Wohnort", 
            fill = "Verbleib in Jahren")
#ggplot_wohnzeit_stadt_rel


grid.arrange(ggplot_wohnzeit_stadt_abs  + theme(legend.position="none"),ggplot_wohnzeit_stadt_rel + theme(legend.position="none"), ncol = 2, bottom = textGrob("Grafik 18: Barplots, Verbleib am Wohnort in Abhängigkeit zum Wohnort || \n Verbleib am Wohnort in Abhängigkeit zum Wohnort - normiert"))
```

Bei der Betrachtung der oberen Plots (`Grafik 18`) erkennen wir in der linken Grafik die tiefere Betrachtung der bereits behandelten „Wohnort der Kunden“. Hierbei haben wir nun die einzelnen Wohnorte weiter nach den Untergruppen des Verbleibes in den Standorten aufgeteilt. Es ist zu erkennen das der Größte Wohnort C in allen Verbleibs-Gruppen ebenfalls die größten Ausprägungen aufweist.
In der rechten Grafik werden die relativen Anteile der Verbleibs-Gruppen in einem Wohnort dargestellt. Hierbei ist zu erkennen, dass die jeweiligen Gruppen über alle betrachteten Wohnorte hinweg in etwa gleich ausgeprägt sind.

Somit können wir aussagen, dass sich die Wohnorte sich in ihrer absoluten Ausprägung deutlich unterscheiden(Wohnort C ist dreimal so groß wie Wohnort A). Die Größe eines Wohnortes hat in unserem Vergleich der drei Standorte aber keine Auswirkung auf die Verweildauer einer Person an diesen Standorten.
...
ggcorrplot(cormat, lab=T) + labs(
       caption = "Grafik 19: Korrelations Matrix, Korrelations Matrix aller Feature")
```
...
Ebenso implementieren wir in unserem Data Pre-Processing eine Skalierung mittels MaxMin-Scaling. Wir skalieren unsere Daten um diese auf ein gleichen Skalenniveau zu bringen und um die Feature miteinandern vergleichbar zu machen. Als Resulat unseres Scalings sind in unserem Datensatz lediglich Werte zwischen 0 und 1 in unserem Scaled-Data-Set. Bei späteren Prognosen müssen diese wieder reskaliert werden.
...
ggplot(head(bf,50000), aes(x=User_ID, y=Purchase, color=Gender, caption = 'Grafik 20: Scatterplot, Scaterplott der ersten 50000 Instanzen')) + 
...
grid.arrange(plt_unscale, plt_scaled, ncol=2, bottom = textGrob("Grafik 21: Histogramme, Histogramm unskaliert || \n Histogramm Purchase skaliert"))
```

Für die Überprüfung auf Normalverteilung mithilfe des Shapiro-Wilk Test ist die Sample Size zu groß ist bzw. der Chi-Quadrat Test nur für kategoriale Variablen anwendbar. Auch ein nicht-parametrischer Test wie Kolmogorov-Sminrov Test kommt nicht infrage, da bei einer so großen Sample Size immer die Null-Hypothese abgelehnt werden würde. Deshalb kann hier gemäß des zentralen Grenzwertsatzes Normalverteilung angenommen werden.
...
```{R}
bf <- bf[sample(nrow(bf), 20000), ]
```

```{R,echo = F}

ggplot(bf, aes(x=Purchase)) + 
  geom_histogram(color ="#E69F00",fill="white") +   
  theme(
    legend.position="none", 
    plot.caption= element_text('Grafik 22: Histogramm, Histogramm der Purchases')
    )
```
...
Im Folgenden unterteilen wir den Datensatz in Training und Test Daten. 70% werden als Trainingsdaten zugeteilt, damit ausreichend viele Beobachtungen im späteren Training verarbeitet werden können. 30% werden den Testdaten zugeteilt, um ein repräsentatives Testszenario zu bilden. Wir müssen die Trainingsdaten zu einem guten Train- TestSplit aufteilen, damit wir einerseits für das Training ausreichend viele, verschiedene und representative Samples verarbeiten und das Modell daraus die unterschiedlichen Charakteristika erlernt und auf untrainierte Daten gut generalisieren kann. Diese Generalisierung testen wir mit den Testdaten. Diese müssen auch wieder ausreichen vielfältig sein und die Stichprobe gut repräsentieren können. Aus diesem Grund wählen wir 30% aus der Gesamtpopulation und weisen sie dem Testdatensatz zu. Somit können wir sicher sein, dass die Ergebnisse durch das Testen akkurat sind und in Verwendung zusätzlicher real-world Samples gut generalisiert. 
```{r}
options(scipen=999)
...
Die k-fache Kreuzvalidierung beinhaltet die Aufteilung des Datensatzes in k-Subsets.Für jede vorenthaltene Teilmenge wird das Modell auf alle anderen Teilmengen trainiert. Dieser Prozess ist abgeschlossen, bis die Accuracy für jede Instanz im Datensatz bestimmt ist. Dannach wird eine Gesamtgenauigkeitsschätzung erstellt. 
Bei "wiederholter"repeated"" Kreuzvalidierung wird das Kreuzvalidierungsverfahren n-mal wiederholt, was zu n zufälligen Partitionen der Originaldatensatzes führt. Die n Ergebnisse werden erneut gemittelt, um eine einzige Schätzung zu erhalten.
Die "repeatedcv" kann den Bias im Estimator verringern und eine bessere Schätzung in Bezug auf die Statistiken produzieren, obwohl der beste Weg, den Bias zu Umgehen, wäre, n unabhängige Datensätze zu erstellen und das Training und Testing auf jedem der Datensätze durchzuführen. Dies würde wiederum viel weniger Daten pro Trainings- und Testwiederholung zulassen, wodruch das das Modell möglicherweise nicht so gut lernt.
Wir haben uns für 5 repeats entschieden, um die Berechnungsdauer nicht zu sprengen. 
...
Zum Vergleich aller Modelle wird der RMSE gewählt, daher werden große Errors relativ stark gewichtet. Der RMSE ist ziemmlich hoch. Dieser Wert soll nun in den folgenden Modellen verbessert werden. 

```{R Feature Importance,message = F, warning = F,echo = F}
var_imp <- varImp(model_lin_regression)
plot(var_imp, top = 10,  sub="Grafik 23: Importanceplot, Feature Importance.")
```


Der obige Plot (`Grafik 23: Importanceplot, Feature Importance.`) zeigt die Variable Importance. Das Modell misst den Produkten der Kategorie 1 & 3 besondere Bedeutung zu, d.h. je mehr in diesen Kategorien eingekauft wird, desto höher der Einkaufspreis. Außerdem hat das Feature Geschlecht großen Einfluss. Ist der Einkäufer männlich, wird gemäß dem Modell mehr eingekauft als eine Einkäuferin. Deshalb werden im Anschluss seperate Modelle für weibliche und männliche Customer aufgestellt.
...
plot(model_RF,  sub="Grafik 24: Linienplot, Error über Anzahl der Bäume.")
```

Der obere Plot (`Grafik 24`) zeigt, wie sehr sich der Error im Laufe der getesteten Baeume verringert.
Der untere Plot (`Grafik 25`) zeigt die Verringerung des RMSE durch das Tuning der Variable `mtry`, wobei `mtry` die Anzahl der Variablen ist, die zufaellig pro Split gesamplet werden. Je hoeher `mtry` desto besser der Regressionsbaum. Als optimaler Wert fuer `mtry` wurde hier `10` gewählt. Für eine Verbesserung des RMSE könnte ein noch höherer Wert für `mtry`gewählt werden, das würde aber unsere Rechenleistung sprengen.
...
ggplot(data=eval,
		aes(x=names,y=scores))+
        geom_bar(position="stack", stat="identity" , fill= brewer.pal(4, "Paired"))+
        xlab("Model") + ylab("RMSE")  + labs(caption = 'Grafik 25:  Barplot, Evaluierung der Modelle') +
...
### Vorhersage für durchschnittlichen Customer

Zuletzt soll noch eine Vorhersage mit einem "durchschnittliche" weibichen & männlcihen Customer getroffen werden. Dafür werden zuerst die Durchschnittswerte für alle numerischen Features ermittelt, alle anderen werden zufällig gewählt. 

````{R Prediction,echo = F,results = 'hide',message = F, warning = F}
fem <- subset(bf,Gender == "F")
sapply(fem, mean)
male <- subset(bf,Gender == "M")
sapply(male, mean)
```

```{R,echo = F,message = F, warning = F}

customer_f <- data.frame("F","36-45","14","A","3","1",5.615075,6.931882,3.452640)
colnames(customer_f) <- colnames(fem_training[0:9])
customer_m <- data.frame("M","26-35","5","B","3","0",5.192446,6.719910,3.992419)
colnames(customer_m) <- colnames(fem_training[0:9])

pred<- data.frame(predict(RF_female,customer_f), predict(RF_male,customer_m))
```

```{R}
colnames(pred) <- c("Prediction weiblich", "Prediction männlich")
knitr::kable(pred)
```


Die Vorhersage für den jeweiligen "Durchschnittskunden" zeigen, dass der durchschnittliche weibliche Kunde weniger einkäuft als der durchschnittliche männliche Kunde. Sind die Features wie Geschlecht, Beruf & Wohnort eines gerade browsenden Kunden bekannt, kann im Hintergrund schnell der zu erwartende Einkauf des Kunden berechnet werden und z.B bei einem Erwartungswert von > 8000 entschieden werden, dass diesem Kunden Werbung vorgespielt werden soll.

## Multilabel Klassifizierung mit Neuralnet

Betrachten wir nochmals den Scatterplot (`Grafik 20: Scatterplot, Scaterplott der ersten 50000 Instanzen`), fällt uns bei der Variable `Purchase` auf, dass sich harte Grenzen im Bereich zwischen 17800 und 22000 und ab 22000 aufwärts befinden. Die Gaps zwischen den Grenzen scheinen jedoch viel zu sauber zu sein, als dass sie organische Usergruppenverteilung wiederspiegel. Wir nehmen an, dass es sich hierbei um externe Einflüsse, wie beispielsweiße Kaufangebote (Nimm 3, zahle 2), handelt. Durch Angebote im Verkauf wäre eine solche klare Trennung der Käufer nachvolziebar. Würde die Trennung der Kunden rein organisch zustandekommen, dann müssten die Purchases der Kunden nach oben hinweg kontinuierlich, in ihrem Auftreten in der Gesamtstichprobe kontinuierlich abnehmen. Je höher die die Purchase Summe, desto weniger representative Samples.

Für die weitere Herangehensweise in der Klassifizierung wollen wir aber dennoch die Kunden diesen drei Clustern zuordnen. Somit stellen die Cluster Kunden dar, die gering, mittel und sehr viel einkaufen. Der Mobile Game Markt lebt z.B. von den sogenannten 'Whales', d.h. Kunden die besonders viel Geld für Ingame-Währungen ausgeben. Daher ist dieses Kundensegment besonders attraktiv. Der Grossteil der Kunden befinden sich im untersten Segment.

Jetzt soll ein Klassifizierungsmodell für die Cluster gefittet werden. Dafür wird als erstes die Variable `Purchase` in 3 Faktoren `low`,`medium`& `high` gebinnt. Im Anschluss wird ein multilable Classification Model mithilfe eines Neural Networks modelliert. 
...
Die Confusion Matrix zeigt die Anzahl der richtig kategorisierten Instanzen. Durch die Ãberrepraesentation der Kategorie `low` wurde nur wenige Instanz aus der Kategorie `medium` und gar keine aus der Kategorie `high` richtig geschätzt, was allerdings trotzdem zu einer hohen Accuracy von >90% führt.  Die Verteilung der Klassen im Verhaeltnis von 190:16:1 (low:medium:high) ist sehr unausgewogen, was zu einem hohen Bias des Modells führen kann. Auch sind die Kategorien nicht äquidistant, sonder aufgrund der visuell bestimmten Cluster gewählt worden. Da Aufgrund von Kapazitaetsproblemen, nicht der gesamte Datensatz als Trainingssatz verwendet werden konnte, koennte die Accuracy noch weiter verbessert werden, da sich die Performance von Neurnalen Netzen mit zunehmenden Trainigsdaten steigert. Für eine weiterführende Analyse sollten die Klassen ausbalanciert werden, um den Bias zu vermeiden.

```{R}
table(training$Purchase)
```
Die Verteilung der Faktoren in Purchase zeigt, dass die Kategorie `high` mit nur 34 Instanzen unterrepräsentiert ist. Bei einem balancierten Datensatz, würde man aus jeder Klasse deshalb 34 Instanzen sampeln. Das ist aber für das Training eines NNs viel zu wenig. Deshalb wird wieder auf den gesamten Datensatz zurückgegriffen.
...
data[is.na(data)] <- 0
data$Purchase <- cut(data$Purchase,breaks=c(0,17800,22000,23961), labels = c("low","medium","high"))
table(data$Purchase)
```
...
Basierend auf dem gesamten Datensatz ist die Accuracy auf 75% gesunken. Unser Modell generalisiert also nicht so gut wie gehofft, wie es uns der vorherige Test gezeigt hat. Gründe dafür könnten die folgenden sein: 

(1) Die Trainingdaten für das ausbalancierte Training waren zu wenig, um eine gute Generalisierteung zu bewirken. Es spiegelt sich weider, dass `low` sehr oft als `medium` klassifziert wird. Es ist also davon auszugehen, dass die Vielseitigkeit der Daten (`low, medium`) über die geringe Anzahl der Trainingsdaten nicht repräsentiert werden konnte. 

(2) Das Training war nicht ausreichend genug und das Modell zu schwach, was zum underfitting führte.

Zusätzlich enhielt der gesamte Datensatz wiederum den Anteil aller Trainingsdaten, was unseren Test zur Generalisierung verfälschte. Wir haben, das aber vernachlässigt, da die Anzahl der Trainingsdaten im Vergleich zur ganzen Stichprobe nur 1.3% betrug. Würden wir diese explizit aus diesem Test entfernen, dan würde sich die Accuracy um wenige %% weiter veringern.

### Vorhersage für durchschnittlichen Customer

Wie auch mit dem RF-Gender Modell, wird hier eine beispielhafte Vorhersage mit einem beliebigen "Durchschnitsskunden" durchgeführt.

```{R, message = F, warning = F}
nn_pred <- predict(nnmodel2,newdata = data_dummies[500,])
colnames(nn_pred) <- c("low", "medium","high")
nn_pred
```

Der Customer wird also in die Kategorie "low" eingeteilt.

Die Anfangs formulierte Modellierungsfrage konnte anhand von mehreren Modellen beantwortet werden, aufgrund der Verteilung der Daten konnte allerdings keines der obigen Regressionsmodelle eine genaueren Vorhersage des Sales Volumen liefern. Das anfängliche Modell einer lineare Regression konnte durch ein Random Forest Modell sowie durch seperate Modelle fuer männliche und weibliche Customer jedoch verbessert werden. Betrachtet man allerdings das Vorhersage-Problem anstatt als eine Regression als ein Klassifizierungsproblem, kann hier eine Accuracy von über 85% erreicht werden. Die Cluster wurden hier visuell bestimmt. Ein Clustering mithilfe eines Algorithmus z.B. KNN wuerde zu einer schärferen Trennung der Cluster führen. Zur weiteren Segmentierung könnte man zu jedem der Cluster `low`, `medium` & `high` ein eigenes Regressionsmodell fitten.

# Schlusswort
Zusammenfassend kann gesagt werden, dass ein signifikanter Unterschied im Kaufverhalten zwischen männlichen und weiblichen Customern besteht, sowohl in der Wahl der der gekauften Produkte sowie die das Einkaufsvolumen. Nach dem wir unsere Daten deskriptiv analysiert und explorative aufbereitet haben, konnten wir tiefergreifendere Informationen aus unseren Kundendaten beziehen. Diese Projektarbeit gestaltete sich für uns als eine observative Studie. Dabei haben wir `Purchase` als Response-Variable und alle restlichen Features (exkl. `User_ID,  Product_ID`) als Prädiktoren ausgewählt. Die erlangten Erkenntnisse ermöglichten es uns, unsere Hyptothese H:"Die Purchases können durch unsere Verkaufsdaten mit guter Genauigkeit vorhergesagt werden." zu besätigen. Unsere Random Forest Modell (RMSE 3268) performte signifikant besser als unser lineares Modell (RMSE 4655). Weitgehend ist der RMSE vom Datensatz abhängig, welcher in unserem Fall sehr unbalanciert ist und, dahergehend die Vorhersagegenauigkeit limitiert.

# Weitere Herangehensweise
Unsere weitere Herangehensweise wäre drei individuelle Trainingsdatensätze aus den drei Clustern (`high, medium, low`) zu samplen. Dadurch könnten wir Random Forest Modelle trainieren die darauf spezialisiert sind, die Purchases der jeweiligen Cluster best möglich vorherzusagen. Unsere Vermutung ist, dass sich der RMSE dadurch noch weiter senken ließe, indem wir Testdaten zuerst über das Neuronale Netzwerk in die drei Cluster (`high, medium, low`) klassifizieren und dann dem entsprechenden Random Forest Modell (trainiert auf Cluster `high, medium, low`) übergeben.
Aus unserer Sicht hätte das jedoch den Scope dieses Projektes überschritte und wir beschlossen uns diese Idee gedanklich durchzuspielen aber nicht umzusetzen.